DELIMITER / / -- Vincolo di controllo sulla data di lancio del satellite
CREATE TRIGGER check_data_lancio_satellite BEFORE
...
END IF;
END / / -- Vincolo di controllo sulla tipologia di guasto
CREATE TRIGGER check_tipo_guasto BEFORE
...
END IF;
END / / -- Vincolo di controllo sulla data di inizio e fine di una missione
CREATE TRIGGER check_data_missione BEFORE
...
END IF;
END / / -- Vincolo di controllo sulle dimensioni dei satelliti
CREATE TRIGGER check_dimensioni_satellite BEFORE
...
END IF;
END / / -- Vincolo di controllo sulla relazione tra tipo operatore e operatore
CREATE TRIGGER check_tipo_operatore BEFORE
